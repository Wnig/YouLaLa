  ...
  getUserData(res2) {
    ...
    if (res2.status == 1) {
      ...
      if (_this.data.page == 'detail') {
        console.log('从商品详情进来:::::');
        let url = '../selCourse/selCourse?sku=' + JSON.stringify(_this.data.skuInfo) + '&type=' + _this.data.type + '&shopid=' + _this.data.fansUserShopId + '&less=' + _this.data.less + '&formid=' + _this.data.userFormId + '&ordertype' + _this.data.ordertype + '&platuserid=' + _this.data.platuserid;
        wx.redirectTo({
          url: url
        });
      };
